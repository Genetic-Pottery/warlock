...
and `## Where to look`: routes from a question to the file, and the name in
it, to open. Warlock lays that out itself from a model's answer, and checks
the answer against the directory before writing it, so a line cannot name a
file or a symbol that is not there. Start at the nearest document above the
code you are about to touch, follow `## Where to look` and the file lines
down, and open the file they name.
