...
#[test]
fn a_file_in_a_tests_directory_is_a_test_file_whatever_it_is_called() {
    // Rust names a split-out test module for what it tests, so the name says
    // nothing and there is no `#[cfg(test)]` block left inside it to find. The
    // directory is the only thing that knows, and without it every one of these
    // reaches a pass whole.
    let source = "\
use super::*;
...
";
    ...
    let elided = elide(Path::new("src/tests/pact.rs"), source).expect("a test file is elided");
    assert!(
        elided.text.contains("lines of test bodies elided"),
        "{}",
        elided.text
    );
    assert!(
        !elided.text.contains("assert_eq!(a + b, 3)"),
        "the body is gone, not merely shortened: {}",
        elided.text
    );

    assert!(
        elide(Path::new("src/pact.rs"), source).is_none()
            || !elide(Path::new("src/pact.rs"), source)
                .expect("checked")
                .text
                .contains("lines of test bodies elided"),
        "the same bytes outside a tests directory are not treated as a test file",
    );
}
