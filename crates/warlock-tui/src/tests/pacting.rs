...
impl Scratch {
    fn new(name: &str) -> Self {
        ...
        let root = env::temp_dir().join(format!("warlock-pact-{}-{name}-{unique}", process::id()));
...
fn load(scratch: &Scratch) -> (App, Scope) {
    ...
    let repo_root = repository_root(tree.root_path()).expect("the load found a repository root");
...
fn a_manifest_that_cannot_be_saved_leaves_the_previous_one_alone() {
    ...
    fs::set_permissions(&manifest_dir, fs::Permissions::from_mode(0o755)).expect("chmods it back");
...
fn events_of(scratch: &Scratch, work: &Work, agent: &dyn Agent, cancel: &Cancel) -> Vec<PactEvent> {
...
fn the_pact_key_starts_nothing_while_a_pact_is_in_flight() {
    ...
    let toggle = pact_press(&mut app, false, Instant::now()).expect("a directory can be pacted");
...
fn a_press_on_a_row_the_ignore_file_keeps_out_starts_nothing() {
    ...
    // answer the key.
    let tree = Tree::new(
        Node::new("/repo", None::<PathBuf>, NodeState::Unpacted).with_children([Node::new(
            "/repo/notes",
            None::<PathBuf>,
            NodeState::Unpacted,
        )
...
fn a_cancel_stops_the_descent_and_records_only_what_finished() {
    ...
    let agent =
        Canned::new(&scratch, ["crates/alpha/src"]).cancelling_at("crates/alpha", cancel.clone());
...
fn a_refresh_counts_what_it_will_visit_rather_than_the_subtree_it_was_pointed_at() {
    ...
    assert_eq!(
        ...
        Some("pact finished — 7 directories, 4:10, $0.00 (incomplete: 7 passes reported no cost)"),
...
mod watching {
    ...
    fn a_trigger_during_a_pact_waits_for_the_runs_own_reload() {
        ...
        let Some(Reloaded(reloaded)) = pact.keep_up(&mut app, &mut manifest, &scope, ended) else {
