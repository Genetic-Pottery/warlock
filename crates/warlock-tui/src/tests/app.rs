...
fn a_row_kept_out_by_the_ignore_file_is_refused_and_nothing_moves() {
    let mut app =
        select(App::from_rows(rows_with_one_kept_out()), "repo/notes").with_counts(StateCounts {
            unpacted: 2,
            pacted_stale: 1,
            ..StateCounts::default()
        });
...
fn a_re_seat_carries_the_collapsed_set_including_paths_the_new_tree_lacks() {
    let app = App::from_tree(&fixture::tree()).with_collapsed(["warlock/crates", "warlock/gone"]);
