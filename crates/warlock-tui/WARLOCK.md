...
- **The purity boundary is the whole design of `src/`.** Only one file (`claude.rs`) spawns a process; only a small named set of others touch a real terminal, clock, or OS handle. Everything else is a pure function of its arguments, with the current instant always threaded in rather than read internally.
- **Two agents share the one process-spawning seam.** `ClaudeAgent` implements the engine's `Agent` port for pacts; `ChatAgent` is a sibling, not an `Agent`, used for read-only conversation turns granted only `Read`/`Grep`/`Glob`, never write.
- **Modal gates and the composer layer in a fixed order** ahead of the ordinary keys: Ctrl-C, then quit confirmation, then the scope prompt, then the write-path prompt, then the composer, then everything else.
