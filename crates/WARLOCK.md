...
- **A stale node stays stale until repacted or refreshed** — `r` now covers the narrower, per-directory case, so the fact to check before assuming a gap is which of `p` (whole subtree) or `r` (only the already-stale members of one) applies to the change at hand.