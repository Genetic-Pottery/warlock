...
- **`warlock-tui`** is the terminal front end: the `warlock` binary plus a `warlock_tui` library. It renders whatever `warlock-engine` computes and translates keystrokes into engine calls (chiefly `p` to pact a subtree, `s` to write a scope directly). It owns the one place a `claude` subprocess gets spawned, but only as a transport for prompts the engine composed.
...
- **A stale node stays stale until repacted, and there is no refresh pass yet** — this is an engine-level fact, not something to rediscover independently while touching workspace config.