...
- **`warlock-engine`** is the domain library: the three-state freshness vocabulary, the tree shape, the manifest format, the subtree hash, the pure staleness decision, the directory loader, the model-pass seam, and the pact/refresh/un-pact operations that write documents and grant freshness. It is deliberately headless — no TUI, terminal, HTTP, or Anthropic dependency is permitted in it, even transitively. Nothing in it enforces scope/sigil boundaries; it only defines what a valid scope or sigil looks like.
- **`warlock-tui`** is the terminal front end: the `warlock` binary plus a `warlock_tui` library. It renders whatever `warlock-engine` computes and translates keystrokes into engine calls — `p` pacts (or un-pacts) a whole subtree, `r` re-runs only the already-stale directories within a pacted subtree, `s` writes a scope directly into the manifest with no worker or reload. It owns the one place a `claude` subprocess gets spawned, but only as a transport for prompts the engine composed. Its own README has drifted behind its `src/`'s documentation (a composer, thread, and viewing/editing modules the README doesn't mention), so its source and `src/`'s `WARLOCK.md` are the trustworthy account there, not the README.
...
- **Don't infer behavior from this directory.** It is build plumbing; the actual design (the three-state model, manifest format, key bindings, view architecture, purity boundaries) lives in the two crates' own READMEs and `WARLOCK.md`s, and the source is authoritative over either where they disagree. Read those before making a change that isn't purely about workspace wiring.
...
- **A stale node stays stale until repacted or refreshed** — `r` covers the narrower, per-directory case within an already-pacted subtree, while `p` handles the whole subtree (and un-pacts one already pacted). The fact to check before assuming a gap is which of the two applies to the change at hand.