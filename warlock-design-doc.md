...
The project tree is the interface. It renders only pacted nodes plus their unpacted surroundings.
...
A pact is the boundary of what Warlock manages. Pacting is incremental: bless as you go.
...
**The adoption ratchet.** Pact expansion only ever points outward, so coverage only grows. You pact ApiServer, the auth dependency forces an expansion, auth's dependencies are the next tickets' crossings, and so on. A real codebase's dependency graph is connected, so following the work pulls the whole project under management over time. Nobody ever decides "let's Warlock the whole repo"; it is a side effect of doing tickets. Every expansion happens at the moment there is concrete work needing it, so no module is ever pacted speculatively. Adoption is not sold, it is produced.
...
A TUI that:
1. renders the pacted tree,
2. colors stale nodes from the hash-based freshness check,
