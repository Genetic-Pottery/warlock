...
**What an operator holds is deliberately not a fourth source of truth.** The sigils on a person's machine (section 8) are operator config: a set of strings saying who is at this keyboard, which says nothing about what the system is, what should happen to it, or why any call was made. They are excluded from the table on purpose, so the next person to notice the gap finds a decision rather than an omission.
...
**A scope is a term of a pact, not a fourth member of that identity.** Node, directory, pact and `WARLOCK.md` remain the same thing. A scope (section 8) qualifies the pact rather than joining the list, which is exactly why only a pacted directory can carry one and why un-pacting takes it away with everything else the pact held. The tree shows a directory's own scope as a label on its row, and that label takes the row's color rather than introducing one of its own, because "there is deliberately no fourth color" is a promise that has to survive every feature that wants to say something on a row.
...
**A crossing defined only against unpacted modules cannot fire at full coverage.** Read "crossings are events, not errors" and the three checkpoints against the greenfield path at the top of this section: pact everything on day one and there are no unpacted modules left, nothing unpacted means nothing ever crosses anything, and all three checkpoints then pass everything forever while the diagnostic below can never fire. The boundary does not break, it is satisfied, and it goes quiet for exactly the team that adopted hardest. One line was doing two jobs. *What Warlock documents* and *what a piece of work may touch* are different sets that coincide only while coverage is partial, and scopes separate them. The checkpoints stand as written where coverage is partial; where it is total they ask their question against scopes instead of against the pacted/unpacted line.

**Scopes and sigils.** A **scope** is a string carried by a pacted directory and committed to the repository, so everyone who clones it can see the boundary — a boundary only some people can see is a rumour rather than a boundary. A **sigil** is a string on one person's machine, entered once, saying who they are; identity belongs where identity lives, and reversing either home is the main way this gets built wrong. A directory has at most one scope while a person may hold many, and that asymmetry is the design rather than an accident of it: a boundary is one answer to "whose is this", while a person can be on two teams. Any one held sigil that matches a directory's scope opens it, so access is a membership test rather than an expression to evaluate. `*` is settled vocabulary on both sides of that pair: held as a sigil it means "may work anywhere", and on the directory side blank already means open to anyone, so `*` is rejected there rather than admitted as a second spelling of a meaning that already has one. Nesting resolves to the nearest ancestor rather than accumulating: a path takes the scope of the closest directory at or above it that has one, and a nearer scope replaces an outer one instead of adding to it, which makes an outer scope a default and never a second gate. Defaults are permissive in both directions — a pacted directory with no scope is open to anyone, and a person holding no sigils is unrestricted, which is every Warlock user today and every one who ignores this tomorrow. And a crossing stays what this section already calls it: an event to warn about, not an error to block. Section 3 governs here as everywhere. Warlock says plainly that a piece of work reaches a scope its operator does not hold, and then gets out of the way.
